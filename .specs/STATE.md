...
- **Agente bloqueado tende a caçar workaround — a regra precisa ser dura.** No 1º start do Analista sem o MCP do Roadmap configurado, ele **reconheceu** a regra "PARE e reporte" e mesmo assim começou a considerar caminhos alternativos: procurar API key no workspace, tentar a API do Paperclip (`127.0.0.1:3100`), achar um servidor local do Roadmap. A instrução existia mas era fraca. Reforçada com uma **"condição de PARADA dura"** no `analista-requisitos/TOOLS.md`, `HEARTBEAT.md` e `SOUL.md`: ausência da tool = fim da linha, sem caçar credencial nem caminho alternativo. → Reforça a contenção (AD-002/003/009) e a questão em aberto do `PAPERCLIP_API_KEY` (o agente alcança o control plane do Paperclip e precisa ser contido por instrução + run policy).
